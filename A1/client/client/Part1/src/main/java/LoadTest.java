...
public class LoadTest {
    ...
    public static double loadTest(int threadGroupSize, int numThreadGroups, int delay, String ipAddr){
        ...
        for (int i = 0; i < 10; i++) {
            initExecutor.execute(new ApiTask(postUri, getUri, 100, initLatch));
        }
        ...
        for (int i = 0; i < numThreadGroups; i++) {
            for (int j = 0; j < threadGroupSize; j++) {
                executor.execute(new ApiTask(postUri, getUri, 1000, latch));
            }
